...
- **On Cowork, the open's capability check is a file listing, never `git status`.** D058 forbids every `git` command from the bridge VM, reads included, and the s32 open broke it seven times before the record had been read -- the capability check ran first, and the rule lived in a file the open had not reached yet. With the record now loaded through the index at step 3, D058's title is one grep away, but the ordering hazard is structural: step 1 runs before step 3. So the check on this surface is `ls "$HOME/mnt/traxgen/allostatik"`, and git state comes from a command handed to the Mac (or a cloud clone of origin for reads). *Test (state):* no `git` in the session's `device_bash` calls; `.git/index.lock` absent on the Mac at close. (Added 2026-09-02, s32.)

- **On Cowork, read the cloud clone with `sed`/`grep` through Bash, never the `Read` tool.** The `Read` tool injects `CLAUDE.md` and its `@`-imports -- the whole canonical set, ~150KB -- into context on its first use inside the clone, and re-reads it every turn after. `environment.md`'s fifth-bridge note measured that in s28; s33 paid it on its first `Read` of `graph.py`, because that note lives in a file the open does not require. This line exists so the rule sits in the file the open *does* load, the same reason the D058 note above does. *Test (state):* no `Read` call on a path under the clone in the session's transcript. (Added 2026-09-09, s33.)

- **A render run is `render_course.py --fresh`, or a campaign inside `scripts.emulator.session()` -- never a render against whatever emulator happens to be up.** The session kills anything running, cold-boots, brings the app to a *recognised* main menu (`wait_for_main_menu`, a signature match, bounded), renders, and tears down in `finally`. Two failures this closes, both s32: a render on a day-old emulator met a `bad color buffer`, and a render on a fresh cold boot met the build tutorial because the fixed 35s settle was shorter than the app's splash. The close step below (kill first) stays and is idempotent with this -- a session that ended cleanly leaves it nothing to do. Campaign scripts still carry their own reset-plus-`SETTLE_SECONDS`; they adopt the session in plan item 16. *Test (state):* the run's output ends with `PASS emulator_down`. (Added 2026-09-07, s32.)

- **Boot the emulator at session open, cold, and only if the session will render.** `uv run python -m scripts.emulator boot` (s30) — the two properties that matter are built into the script rather than left to whoever types the command. The wait for `sys.boot_completed` is **bounded** and does not suppress stderr — an unbounded `until` with `2>/dev/null` spins forever on an `adb` that is not on `PATH`, observations #25 *is* that exact command, and the timeout here carries whatever adb last said. And it grades the device immediately after, so the device being up is a checked postcondition rather than an assumption — but with preflight's **four device checks**, not all six (the app-version check is device-level, s33): right after a cold boot the phone launcher is in front, so `app_in_foreground` fails and `screencap_geometry` measures the launcher (`environment.md`, Gotchas). Those two are campaign-time, which is what the earlier wording here did not say. It also prints the measured boot time, which `decisions.md` says has never been recorded on this AVD. Then run preflight again before **every** campaign rather than once at open: per-session cycling fixes the overnight cost and does nothing about drift *within* a session, and a cold boot starts the `bad color buffer` count at 0, which makes any nonzero value later unambiguous. A session that will not render should not boot it at all.

- **The adversarial panel is standing procedure here, and it is the session's largest single cost.** Run it on work that is finished, green, self-reviewed, and about to be committed — that is exactly the state in which it has paid, four sessions running (observations #19). Scope it to the diff, never the repo. Skip it for doc-only edits, mechanical refactors the suite already covers, and anything a clean revert undoes. Three lenses is the shape that has worked: epistemics (is every checkable sentence true?), mutation (what coincidence do the fixtures share, and what mutation exploits it?), and design (what does this change let a caller do that was previously impossible, and is the code that *consumes* it graded?). *Measured 2026-08-24 (s25):* one panel was ~4.5M effective tokens, **~21% of the whole session** — twice the instructions-and-tools line, and second only to re-reading the conversation itself — and it found a false ERROR in code that was already green and self-reviewed. Both facts are true; the second does not excuse ignoring the first. See the global preferences' *fresh adversarial reviewer* bullet for the general rule. **The mutation lens runs on its own worktree, or last** (s33, #44): three lenses in parallel on one clone had the mutation lens editing `graph.py` under the other two, and both of them spent effort on phantom failures before finishing.
