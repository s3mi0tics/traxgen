...
- **Explain testing concepts rather than assuming them.** Colby is new-ish to automation testing (finishing a Playwright course as of mid-2026). `pytest` is what this project uses; when a testing idea shows up — fixtures, property-based testing, fakes and injection, synchronization strategy — explain the concept alongside the code rather than treating it as background. Global preferences already say "teaching mode, calibrated to current capability"; this is the calibration datum for *this* domain, and it is time-limited — fade it as fluency shows. **Re-fired 2026-08-21 (s22): density crept above level as the probe/panel theory evolved, and Colby flagged it mid-session ("the language is getting a little above my head"). Recalibrate down; do not fade yet.**
- **Don't force Playwright in where it doesn't fit.** Being mid-course is not a reason to reach for browser automation: traxgen is a Python backend library and its test story is pytest + hypothesis. Borrowing Playwright's *concepts* is fair game and useful (2026-08-07: its auto-waiting model framed the harness's fixed-sleep problem exactly); importing the tool is not.
